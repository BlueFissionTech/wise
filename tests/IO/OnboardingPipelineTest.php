...
final class OnboardingPipelineTest extends TestCase
{
    private string $root;
    ...
    public function testOnboardScriptPromptsAndSummarizes(): void
    {
        ...
        $kernel->handle('onboard');
        $output = $kernel->lastOutput();

        $this->assertStringContainsString('Wise onboarding begins.', $output);
        $this->assertStringContainsString('User: alice', $output);
        $this->assertStringContainsString('Role: dev', $output);
        $this->assertStringContainsString('Focus: engineering', $output);
        $this->assertStringContainsString('Goal: ship a prototype', $output);
    }

    private function makeKernel(array $responses): OnboardingTestKernel
    {
...
final class OnboardingTestKernel extends Kernel
{
...
final class OnboardingNullDisplayDriver implements IDisplayDriver
{
